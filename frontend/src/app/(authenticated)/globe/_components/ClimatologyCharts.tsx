...
export default function ClimatologyCharts({ lat, lon, targetDate, buffer = 3, startYear = 2000, endYear = 2024, param = 'PRECTOT' }: Props) {
	...
	return (
		<div className="space-y-4">
			{loading && <div className="text-muted-foreground text-sm">Loading climatology...</div>}
			{error && <div className="text-red-400 text-sm">{error}</div>}
			<div className="bg-card/90 p-3 rounded-lg">
				<div className="flex justify-between items-center mb-2">
					<div className="font-semibold text-sm">Historical (±{buffer} days) — multi-year{usedParam ? ` · ${friendlyParamName(usedParam)}` : ''}</div>
				</div>
				...
				<div className="mt-2 text-muted-foreground text-xs">Median (day 0): {Number.isFinite(medianSummary.medOfMeds) ? `${medianSummary.medOfMeds} ${unitsFor(usedParam)}` : 'n/a'} — IQR: {medianSummary.iqrCenter || 'n/a'}</div>
			</div>
			<div className="bg-card/90 p-3 rounded-lg">
				<div className="mb-2 font-semibold text-sm">Distribution (±{buffer} days)</div>
